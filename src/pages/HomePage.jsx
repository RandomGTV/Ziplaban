    ...
    <div className="home-content">
      <section className="home-section home-signatures" aria-labelledby="home-signatures-title">
        ...
        <div className="home-signature-grid">{signatures.map((product, index) => <article className="home-signature-card" key={product.id}>
          ...
          <div className="home-card-bottom"><div><h3><a href={`/product/${product.id}`} onClick={event => link(event, `/product/${product.id}`)}>{['Koshari Trio', 'Salankatiya', 'Ruh Hayathi'][index]}</a></h3><p>{money(product.price)} <span>/ Regular</span></p></div><button className="home-add" aria-label={`View ${product.name}`} onClick={() => navigate(`/product/${product.id}`)}><Plus size={22}/></button></div>
        </article>)}</div>
      </section>
      ...
      <section className="home-flavours" aria-labelledby="home-flavours-title" style={{ '--flavour-color': selection.color }}>
        <div className="home-flavour-layout home-section">
          <div className="home-flavour-copy"><span className="home-kicker">FOLLOW YOUR CRAVING</span><h2 id="home-flavours-title">What’s your<br/>happy flavour?</h2><div className="home-flavour-options" role="group" aria-label="Choose a flavour">{flavours.map((item, index) => <button key={item.label} aria-pressed={index === flavour} onClick={() => setFlavour(index)}>{item.label}</button>)}</div><div aria-live="polite"><h3>{selection.title}</h3><p>{selection.copy}</p></div><button className="home-pill" onClick={() => navigate(`/product/${featured.id}`)}>Explore {featured.name} <ArrowRight size={18}/></button></div>
