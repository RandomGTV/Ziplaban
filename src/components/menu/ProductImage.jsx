...
export default function ProductImage({ product, className='', priority=false }) {
  ...
  if (product.imageUrl) return <div className={`menu-product-image ${!loaded&&!failed?'is-loading':''} ${className}`}>
    {failed ? <span className="menu-image-fallback">{product.name}</span> : <img src={product.imageUrl} alt={product.name} loading={priority?'eager':'lazy'} decoding="async" onLoad={()=>setLoaded(true)} onError={()=>setFailed(true)} />}
  </div>;
