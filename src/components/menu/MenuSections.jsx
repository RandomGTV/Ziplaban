...
export function MenuHero({onBrowse}) {
  ...
  return <section className="menu-hero">
    <div className="menu-hero-inner menu-container">
      <div className="menu-hero-copy"><p className="menu-eyebrow"><span/> ZIP LABAN MENU</p><h1>What Are You<br/><em>Craving</em> Today?</h1><p className="menu-hero-subtitle">Creamy favourites, crunchy toppings<br/>and happiness in every bowl.</p><div className="menu-hero-buttons"><button className="menu-primary menu-primary--white" onClick={onBrowse}>View All Desserts <ArrowDown size={18}/></button><button className="menu-text-button" onClick={()=>navigate('/build-your-bowl')}>Build Your Bowl <ArrowRight size={18}/></button></div><div className="menu-hero-promise"><Leaf size={15}/><span>Made fresh.</span><Heart size={15}/><span>Made for you.</span></div></div>
...
export function FeaturedProduct({product,onQuickAdd}) {
  return <section className="menu-featured"><div className="menu-featured-food"><span className="menu-featured-watermark" aria-hidden="true">FAVOURITE</span><ProductImage product={product}/><img src="/images/zip_boy_mascot.png" alt="" className="menu-featured-mascot" loading="lazy"/></div><div className="menu-featured-copy"><span className="menu-eyebrow"><Sparkles size={15}/> ZIP LABAN FAVOURITE</span><span className="menu-featured-badge">THE ONE EVERYONE LOVES</span><h2>Koshari<span>One bowl. All the happiness.</span></h2><p>Creamy layers. A generous crunch. Pistachio, Nutella and Lotus coming together in one very happy bowl.</p><div className="menu-ingredients">{product.ingredients.map(item=><IngredientChip key={item}>{item}</IngredientChip>)}</div><div className="menu-featured-order"><strong>{money(product.price)}</strong><button className="menu-primary menu-primary--white" onClick={()=>onQuickAdd(product)}>Try It Now <ArrowRight size={18}/></button></div></div></section>;
}
...
export function NewArrivalsCarousel({onQuickAdd}) {
  ...
  return <section className="menu-arrivals-section"><div className="menu-container"><div className="menu-section-heading"><div><span className="menu-eyebrow">FRESH ON THE MENU</span><h2>Something New<br/><em>Just Landed.</em> <Sparkles/></h2></div><div className="menu-carousel-controls"><button aria-label="Previous new arrivals" onClick={()=>track.current.scrollBy({left:-330,behavior:reduce?'auto':'smooth'})}><ArrowLeft size={20}/></button><button aria-label="Next new arrivals" onClick={()=>track.current.scrollBy({left:330,behavior:reduce?'auto':'smooth'})}><ArrowRight size={20}/></button></div></div><div className="menu-arrival-track" ref={track} tabIndex={0} aria-label="New arrivals carousel">{MENU_ARRIVALS.map(product=><article className="menu-arrival-card" key={product.id}><span className="menu-arrival-new">JUST LANDED</span><ProductImage product={product}/><div><h3>{product.name}</h3><strong>{money(product.price)}</strong></div><button onClick={()=>onQuickAdd(product)} aria-label={`Add ${product.name} to order`}><ArrowRight size={20}/></button></article>)}</div><button className="menu-arrival-explore" onClick={()=>navigate('/new-arrivals')}>Explore New Arrivals <ArrowRight size={18}/></button></div></section>;
}

export function BuildYourBowlCTA() {
  const {navigate}=useNavigation();
  return <section className="menu-builder menu-container"><div><span className="menu-eyebrow">A LITTLE MORE YOU</span><h2>Your Bowl.<br/><em>Your Rules.</em></h2><p>Pick your base, sauce, crunch and toppings<br/>and build your own ZIP LABAN happiness bowl.</p><button className="menu-primary" onClick={()=>navigate('/build-your-bowl')}>Build Your Bowl <ArrowRight size={18}/></button><div className="menu-builder-steps"><span>01 Base</span><span>02 Sauce</span><span>03 Crunch</span><span>04 Happiness</span></div></div><div className="menu-builder-art"><img src="/images/zip_boy_mascot.png" alt="Zip Laban mascot inviting you to create your bowl" loading="lazy"/><div className="menu-empty-bowl"><div className="menu-bowl-inside"/><div className="menu-bowl-brand" aria-label="Zip Laban"/></div><span className="menu-floating-label menu-floating-label--one">Pistachio ♡</span><span className="menu-floating-label menu-floating-label--two">Extra crunch!</span><span className="menu-floating-label menu-floating-label--three">Chocolate</span><FloatingNut className="menu-builder-nut"/></div></section>;
}
